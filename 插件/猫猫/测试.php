...
if ($this->事件类型 == '群消息' && $this->消息内容->纯文本 == '取') {
    ...
    $发包 = json_decode($this->发包("trpc.msg.register_proxy.RegisterProxy.SsoGetGroupMsg", $packet), true);
...
if ($this->事件类型 == '群消息' && $this->消息内容->纯文本 == '取按钮') {
    ...
    $发包 = json_decode($this->发包("trpc.msg.register_proxy.RegisterProxy.SsoGetGroupMsg", $packet), true)[3][6][3][1][2][5];
    $按钮id = $发包[53][2][1][1][1][1];
    $按钮data = $发包[53][2][1][1][1][3][5];
    $官机id = $发包[53][2][1][2];
    $发包 = json_encode($发包, 480);
