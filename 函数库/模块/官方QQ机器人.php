        ...
        switch ($解析["op"]) {
            case 0:
                switch ($解析["t"]) {
                    ...
                    case "GROUP_MESSAGE_CREATE"://全量信息
                        $this->用户ID   = $解析["d"]["author"]["id"];
                        ...
                        $this->信息ID   = $解析["d"]["id"];
                        $this->事件ID   = $解析["id"];
                        echo "[群聊全量信息：{$this->来源ID}]{$this->用户昵称}: {$this->用户信息}\n";
                        break;
    ...
    public function 发送($类型, $主内容=null, $附加1=null, $附加2=null)
    {
        ...
        switch ($类型) {
            ...
            case "MarkDown":
                ...
                if (!empty($附加2)) {
                 // 强制判断：如果 $附加2 是数组，且包含 content 或 rows，就按原生按钮处理
                 if (is_array($附加2)) {
                     // 兼容两种写法：带content层 / 直接是rows数组
            ...
            case "文件":
                ...
                $file_type = array("图片"=>1, "视频"=>2, "语音"=>3, "文件"=>4)[$类型];
